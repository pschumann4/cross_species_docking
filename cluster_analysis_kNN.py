####################################################################################################
# This script will perform a k-nearest neighbors cluster analysis for generating species           #
# susceptibility calls based on the docking metrics of the self-docking pose                       #
# (i.e., reference pose). The user will need to provide a summary file containing the docking      #
# metrics for the self-docking pose and the poses of the test set. The user will also need to      #
# provide the species name and pose number of the self-docking pose.                               #
# This script will output a summary of the species susceptibility calls and plots of the           #
# clustering results.                                                                              #
####################################################################################################
...
def cluster_analysis():
    ...
    grid = GridSearchCV(KNeighborsClassifier(), param_grid, cv=KFold(n_splits=5, shuffle=True, random_state=42), scoring='accuracy')
    ...
    # View the results
    print('The optimal number of neighbors is: {}'.format(grid.best_params_['n_neighbors']))
