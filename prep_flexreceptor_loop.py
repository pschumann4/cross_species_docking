####################################################################################################
# This function loops the prepare_flexreceptor.py script over a specified directory of PDB files   #
...
def prep_flex_receptors():
    ...
    while not os.path.exists(pdbqt_dir):
        pdbqt_dir = input('That path does not appear to exist.\nPlease enter the path to the directory containing the PDBQT files: ')
    ...
    while not os.path.exists(script):
        script = input('That path does not appear to exist.\nPlease enter the path to the prepare_flexreceptor.py script: ')
        if script.startswith('"') and script.endswith('"'):
            script = script[1:-1]
    ...
    while not os.path.exists(residues):
        residues = input('That path does not appear to exist.\nPlease enter the path to the flex_residues.txt file: ')
        if residues.startswith('"') and residues.endswith('"'):
            residues = residues[1:-1]

    # Open the flex_residues.txt file and get the list of PDBQT files and their flexible residues
    ...
    for pdbqt in pdbqt_files:
        if pdbqt.endswith('.pdbqt'):
            ...
            # Match the PDBQT file name to the PDBQT ID in the flex_residues.txt file and get the residues to set as flexible
            for line in res_list:
                if line.split(':')[0] == pdbqt_name:
                    res = line.split('[')[1].split(']')[0]
                    # Create the command to run the prepare_flexreceptor.py script
                    cmd = 'python ' + script + ' -r ' + pdbqt_name + '.pdbqt -s ' + res + ' -g ' + pdbqt_name + '_rigid_residues.pdbqt -x ' + pdbqt_name + '_flex_residues.pdbqt -v'
                    os.system(cmd)
        else:
            continue
    print('DONE!')
