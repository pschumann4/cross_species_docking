...
def get_config_files():
    ...
    os.chdir(pwd)
    # Ask the user for all of the configuration file information. The user will be asked to confirm this information later, so it will be necessary to ask for this information within a loop until the user confirms that it is correct
    while True:
        ligand_name = input("Enter the name of the ligand PDBQT file that will be used for the docking simulation: ")
        ...
        center_x = input("Enter the center coordinate of the gridbox in the x dimension: ")
        center_y = input("Enter the center coordinate of the gridbox in the y dimension: ")
        center_z = input("Enter the center coordinate of the gridbox in the z dimension: ")
    ...
    while True:
        ...
        for file in os.listdir(pwd):
            if file.endswith(".pdbqt") and ligand_name not in file and "residues" not in file:
                # Get the base name of the file
                base_name = file.split('.')[0]
                # Ask the user if the file is flexible or rigid
                is_flex = input("Should " + base_name + " be treated as flexible? (y/n): ")
    ...
    for file in rigid_pdbqt_files:
        with open(file.split('_')[0] + "_conf" + ".txt", "w") as f:
            ...
            f.write("exhaustiveness = {}\n\n".format(exhaustiveness))
            f.write("out = " + file.split('_')[0] + "_bound_" + ligand_name + ".pdbqt")
    # Loop through the file directory and check that there is a config file for each PDBQT file
    for file in os.listdir(pwd):
        if file.endswith(".pdbqt") and ligand_name not in file and "residues" not in file:
