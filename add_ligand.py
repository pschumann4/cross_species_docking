    ...
    for pdb_file in pdb_files:
        ...
        for file in files:
            if file.endswith('.pdbqt') and 'flex' in file and pdb_name in file:
                ...
                # Open new file and store lines in dictionary using residue number and atom name as key
                ...
                output_file = os.path.join(file_directory, output_prefix + str(counter) + '.pdb')
                        ...
                        for line in f:
                            if line.startswith('ATOM'):
                                ...
                                if (residue_name, residue_num, atom_name) in new_lines:
                                    # Replace the coordinates of the original line with the coordinates of the new line
                                    new_line = list(line)
                                    new_line[30:38] = new_lines[(residue_name, residue_num, atom_name)][30:38]
                                    new_line[38:46] = new_lines[(residue_name, residue_num, atom_name)][38:46]
                                    new_line[46:54] = new_lines[(residue_name, residue_num, atom_name)][46:54]
...
def add_ligand_to_flex(file_directory):
    ...
    for pdb_file in files:
        if pdb_file.endswith('.pdb') and 'model' not in pdb_file:
            ...
            # Loop through files in directory that contain the word "ligand" in the file name and match the PDB file name and number
            for file in files:
                if file.endswith(pdb_number + '.pdbqt') and 'ligand' in file and pdb_name in file:
                    # Get file path
                    file_path = os.path.join(file_directory, file)
                    # Create output file path
                    output_file = os.path.join(file_directory, pdb_name + '_flex_model' + pdb_number + '.pdb')
                    ...
                    with open(file_path, 'r') as f:
                        for line in f:
                            if line.startswith('HETATM'):
                                # Remove the segment identifier [70:76] from the line and add 6 space characters in its place
                                line = line[:70] + ' ' * 6 + line[76:]
                                # Remove the active or inactive flags from the element symbol [78:80]
...
def add_ligand_to_rigid(file_directory):
    ...
    for file in files:
        ...
        # Check if the file name is in the list of flexible files and if it is, skip it
            if pdb_name not in flex_files and 'model' not in file:
                # Loop through files in directory that contain the word "ligand" in the file name and match the PDB file name
                for pdb_file in files:
                    if 'ligand' in pdb_file and pdb_name in pdb_file:
                        ...
                        # Create output file path
                        output_file = os.path.join(file_directory, pdb_name + '_rigid_model' + file_number + '.pdb')
                        # Create empty list to store lines
                        lines = []
                        ...
                        with open(file_path, 'r') as f:
                            for line in f:
                                if line.startswith('HETATM'):
                                    ...
                                    line = line[:70] + ' ' * 6 + line[76:]
                                    # Remove the active or inactive flags from the element symbol
                                    line = line[:78] + line[79:]
                                # Store the line in the list unless it's a hydrogen
                                if line[76:78].strip() != 'H':
                                    lines.append(line)
...
def check_chain_ids(pdb_file, ref_pdb, ligand):
    ...
    with open(pdb_file, 'r') as f:
        ...
        for line in f:
            ...
            if line.startswith('ATOM'):
                # Check if the chain identifier is the same as the reference chain identifier
            ...
            if line.startswith('HETATM'):
                # Check if the chain identifier is the same as the reference ligand chain identifier
            ...
            # If the line is not an ATOM or HETATM record, add it to the list of modified records
...
def move_hetatms(pdb_file):
    ...
    with open(pdb_file, 'r') as f:
        for line in f:
            ...
            if line.startswith('HETATM'):
                ...
                hetatms.append(line)
                # Reorder the hetatm records based on the atom name [12:16] in alphabetical order
    ...
    for i, line in enumerate(all_lines):
        ...
        if line.startswith('CONECT') or line.startswith('ROOT') or line.startswith('ENDROOT'):
            all_lines[i] = ''
...
def add_ligand():
    ...
    pwd = input('Enter the directory containing the PDB files and related PDBQT files: ')
    ...
    while not os.path.exists(ref_name):
        ...
        ref_name = input('Enter the name of the reference PDB file (or type "exit"): ')
        if check_exit(ref_name):
            return
    ...
    for file in os.listdir(pwd):
        ...
        if file.endswith('.pdb') and 'flex' in file:
            # Get the base name of the file and append it to the list of flexible files
    ...
    for file in os.listdir(pwd):
        ...
        if file.endswith('.pdb'):
            ...
            # If not in flexible files or the reference model, then add ligand to rigid model
            elif file_name not in flex_files and 'flex' not in file and file != ref_name:
                add_ligand_to_rigid(pwd)
                print('Adding ligand to rigid model: ' + file)

    # Re-loop through the files in the directory to apply the move_hetatms() and check_chain_identifiers() functions
    for file in os.listdir(pwd):
        ...
        # Remove all PDB files that contain that word 'flex' but do not contain the word 'model'
    ...
    print("All PDB models are being moved to the 'models' folder in the working directory...")
