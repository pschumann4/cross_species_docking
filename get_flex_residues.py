####################################################################################################
# This script is used to identify the flexible residues in the binding site of a protein by        #
# comparing the binding site structure to a reference structure. Residues are considered flexible  #
# if they contain an atom that is greater than 3 Angstroms away from the corresponding atom in the #
# reference structure.                                                                             #
####################################################################################################
...
def residue_dist(pdb1, pdb2, dist_cutoff=3):
    ...
    dist_dict = {}
    # Iterate through the lines in the PDB file, create temporary lists to store the coordinates of the residues
    for line in lines1:
        ...
        if line.startswith('ATOM'):
            ...
            for line2 in lines2:
                coords2 = []
                if line2.startswith('ATOM') and line2[22:26].strip() == line[22:26].strip() and line2[13:16].strip() == line[13:16].strip():
                    ...
                    if resid2 == resid1:
                        ...
                        coords2 = [float(line2[30:38]), float(line2[38:46]), float(line2[46:54])]
                        # Calculate the euclidean distance between the two coordinates
                        if coords1 and coords2:
                            dist = euclidean3d(coords1, coords2)
                        # Check if the resid is already in the dictionary, if not, add it. If there is already a distance for that residue, retain the larger distance
...
def compare_pdbs(pdb1, pdb2, ligand):
    # Calculate the minimum distance between the ligand and the residue atoms on a per residue basis in the first pdb file
    ...
    # Filter the list of flexible residues to only include residues that are in the close_residues list
    flex_residues_close = {residue: rmsd for residue, rmsd in flex_residues.items() if residue[3:] in close_residues}
    # Rank the flexible residues by dist value and return the top 10
    flex_residues10 = sorted(flex_residues_close.items(), key = lambda x: x[1], reverse = True)[:10]
...
def get_flex_residues():
    ...
    working_dir = input('Enter the name of the file directory containing the PDB files to analyze: ')
    ...
    pdb1 = input('Enter the name of the reference structure (with the ligand bound): ')
    if not pdb1.endswith('.pdb'):
        pdb1 += '.pdb'
    # Check that the pdb file exists and if not, ask the user to enter the name of the pdb file again
    ...
    for line in lines:
        if line.startswith('HETATM') and line[17:20].strip() == ligand:
            ligand_in_pdb = True
            break
    # If the ligand is not in the pdb file, ask the user to enter the name of the pdb file again
    while not ligand_in_pdb:
        print('This pdb file does not contain the ligand.')
        pdb1 = input('Enter the name of the reference structure containing the ligand or type "exit": ')
        if check_exit(pdb1):
            return
    # Check that the pdb file exists and if not, ask the user to enter the name of the pdb file again
    while not os.path.exists(pdb1):
    ...
    # Get the name of the binding site structure file as the characters before the first underscore
    pdb1_name = pdb1_base.split('_')[0]
    # Loop through the files in the working directory to get the pdb files that are not the reference binding site
    pdb2s = []
    for file in os.listdir():
        if file.endswith('.pdb') and file != pdb1 and pdb1_name not in file:
            pdb2s.append(file)
    # Check that there are pdb files in the working directory that are not the reference binding site
    if len(pdb2s) == 0:
        print('Error: There are no other pdb files in the working directory.')
        return
    
    # Create a dictionary to store the flexible residues using the file name as the key
    flex_residues = {}
    ...
    all_flex_residues = list(set([res for pdb2, residues in flex_residues.items() for res in residues]))
    ...
    with open('flex_residues.txt', 'w') as f:
        for line in lines:
            ...
            line = line.replace(', ', ',')
            f.write(line)
    # Check if a folder called 'details' exists and if not, create it and move the text file into it
    if not os.path.exists('details'):
        os.mkdir('details')
    os.rename('flex_residues.txt', 'details/flex_residues.txt')
    print('\nA summary of these results have been written to the file "flex_residues.txt" and can be found in the "details" folder.')
    return
