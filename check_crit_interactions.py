...
def check_crit_interactions():
    ...
    It will prompt the user for the directory containing the PLIF text files,
    the critical residues, and the type of interaction to check for.
    """
    ...
    aa_dict = {
        ...
        "SEC": "U",
        "PYL": "O",
    }  # Create lists to store the residue info
    restype = []
    resnr = []
    ...
    crit_res = input(
        "Enter a comma separated list of residue "
        "names and numbers (e.g. 'ARG 1, PHE 2, LEU 3'): "
    )
    ...
    for i in range(len(crit_res)):
        if " " not in crit_res[i]:
            ...
            crit_res = input(
                "Enter a comma separated list of residue names "
                "and numbers (e.g. 'ARG 1, PHE 2, LEU 3'): "
            )
            if check_exit(crit_res):
                return
            if ", " in crit_res:
    ...
    # If the user entered single letter amino acid codes,
    # convert them to three letter codes
    ...
    for file in os.listdir(pwd):
        if file.endswith(".txt"):
            ...
            # If the 'query' column of that residue is '1',
            # add the residue to the list
            for i in range(len(df)):
                # Loop over each critical residue
                for j in range(len(restype)):
                    # If the residue name and number match, and the
                    # 'query' column is '1', add the residue to the list
                    if (
                        df["restype"][i] == restype[j]
                        and df["resnr"][i] == int(resnr[j])
                        and df["query"][i] == 1
                    ):
    ...
    sns.heatmap(
        df2, cmap="RdYlGn", annot=False, cbar=False, linecolor="black", linewidths=1
    )
    plt.tight_layout()
