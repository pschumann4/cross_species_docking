...
def update_ligand(ligand_file, ligand_name):
    """
    Updates the ligand file.
    """
    # Create a list to store the new lines
    new_lines = []
    # Open the ligand file and read the lines
    with open(ligand_file, "r") as f:
        for line in f:
            ...
            if line.startswith("ATOM"):
                ...
                # Check if the atom name matches the ligand name
                cur_ligand_name = line[17:20].strip()
                if cur_ligand_name == ligand_name:
                    continue
                # Replace the atom name with the ligand name
                line = line.replace(cur_ligand_name, ligand_name)
            # Remove unwanted records
            if line.strip() and not any(line.startswith(keyword) for keyword in ["ROOT", "ENDROOT", "BRANCH", "ENDBRANCH", "TORSDOF"]):
                new_lines.append(line)
    # Write the new lines back to the ligand file
...
def move_hetatms(pdb_file):
    ...
    for i, line in enumerate(all_lines):
        ...
        if line.startswith("HETATM") or line.startswith("ATOM"):
            ...
            atom_number_str = line[7:11].strip()
...
def add_ligand():
    ...
    while not os.path.exists(pwd):
        pwd = input("That path does not appear to exist.\n"
                    "Please enter the path to the 'vina_output' directory: ")
    ...
    ligand_name = input("Enter the ligand ID as it is found within the reference PDB: ")

    # For all ligand files, update the ligand ID
    print("Updating ligand files...")
    for file in os.listdir(pwd):
        if file.endswith(".pdbqt") and "ligand" in file:
            update_ligand(file, ligand_name)
    ...
    flex_files = [] # List to store the flexible files

    # Loop through the files in the directory
    for file in os.listdir(pwd):
        # Check if the file is a PDB file and if it is a flexible model
        if file.endswith(".pdb") and "flex" in file:
            # Append the file name to the list of flexible files
    ...
    for file in os.listdir(pwd):
        # Check if the file is a flexible PDB
        if file.endswith(".pdb"):
    ...
    for file in os.listdir(pwd):
        if file.endswith(".pdb") and "model" in file:
            ...
            move_hetatms(file)
            check_chain_ids(file, ref_name, ligand_name)
