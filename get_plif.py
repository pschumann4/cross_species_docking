####################################################################################################
# This script is used for generating Protein-Ligand Interaction Fingerprints (PLIFs) from the      #
# .xml output of the Protein-Ligand Interaction Profiler (PLIP) tool.                              #
# The PLIFs include all intermolecular interactions counted by PLIP as well as van der Waals       #
# contacts. This script will also calculate the Tanimoto coefficients for the inputted PDBs.       #
# Each PLIF will be saved as a .txt file along with a heatmap of the PLIF matrix.                  #
# To execute the script, type 'python plif_matrix.py' at the command line and follow the prompts.  #
####################################################################################################
...
def parse_plip_xml(xml_file):
    ...
    if binding_site is not None:
        ...
        for interaction_type in ['hydrophobic_interactions', 'hydrogen_bonds', 'water_bridges', 'salt_bridges', 'pi-stacks', 'pi_cation_interactions', 'halogen_bonds', 'metal_complexes']:
            ...
            # Iterate over the interactions of the current type
                for interaction in interaction_elem:
                    ...
                    if resnr is not None:
                        ...
                        data.append({'resnr': resnr, 'restype': restype, 'interaction_type': interaction_type})
...
def merge_dfs(df0, df1):
    ...
    df0 = df0.rename(columns={"resnr": "resnr_1", "restype": "restype_1", "interaction_type": "interaction_type_1"})
    df1 = df1.rename(columns={"resnr": "resnr_2", "restype": "restype_2", "interaction_type": "interaction_type_2"})
    # Merge and add new columns that state T/F depending on if the row value is unique
    merged_df = pd.merge(df0, df1, left_on=['resnr_1', 'restype_1', 'interaction_type_1'], right_on=['resnr_2', 'restype_2', 'interaction_type_2'], how='outer')
    merged_df = merged_df.assign(X=merged_df['resnr_1'].notnull(), Y=merged_df['resnr_2'].notnull())
    ...
    merged_df = merged_df.rename(columns={"resnr_1": "resnr", "restype_1": "restype", "interaction_type_1": "interaction", "X": "ref", "Y": "query"})
    merged_df[["ref", "query"]] = merged_df[["ref", "query"]].astype(int)

    return merged_df

def similarity_plifs(plif_ref, plif_query):
    # Calculate Tanimoto Sim using RDkit function 
...
def get_vdw_contacts(pdb_file,ligand_name):
    ...
    df = pd.DataFrame(columns=['HETATM', 'LIG x', 'LIG y', 'LIG z', 'ATOM', 'restype', 'resnr', 'RES x', 'RES y', 'RES z', 'DIST','vdw_radii','vdw_interaction'])
    ...
    # Loop through each row of the protein and ligand coordinates and calculate euclidean distance using euclidean3d function
    ...
    for i, protein_row in enumerate(protein_coordinates):
        ...
        for j, ligand_row in enumerate(ligand_coordinates):
            ...
            # Threshold set to 4.5 angstroms to ensure maximum vdw_radii are retained with additional atoms
            if dist < 4.5:
                temp_df = pd.DataFrame({'HETATM': [ligand_row[0]],
                                       'LIG x': [lig_x],
                                       'LIG y': [lig_y],
                                       'LIG z': [lig_z],
                                       'ATOM': [protein_row[0]],
    ...
    df['vdw_radii'] = df.apply(lambda row: vdw_radii.get(row['HETATM'][0], 0) + vdw_radii.get(row['ATOM'][0], 0) + 0.6, axis=1)
    df['vdw_interaction'] = df.apply(lambda row: True if row['DIST'] < row['vdw_radii'] else False, axis=1)
    
    vdw_df = df[df['vdw_interaction'] == True]
    vdw_df = vdw_df.groupby(['resnr','restype']).first().reset_index()
    vdw_df = vdw_df[['resnr', 'restype']].rename(columns={'resnr': 'resnr', 'restype': 'restype'}).assign(interaction_type='vdw_contact')
    return vdw_df

def sim_matrix(tanimoto_dict):
    # Split the dictionary keys into sets
    keys = list(set([i.split(' & ')[0] for i in tanimoto_dict.keys()] + [i.split(' & ')[1] for i in tanimoto_dict.keys()]))
    ...
    # Create a heatmap with a colorbar gradient that uses a colorblind friendly color scheme
...
def PLIP_PLIFs():
    ...
    while not ref_pdb.endswith('_protonated.pdb'):
        print('Error: The reference PDB file name must be protonated and the file name must end with "_protonated.pdb".')
    ...
    while not os.path.exists(ref_pdb):
        print('Error: This file does not appear to exist. Make sure that the file name ends with "_protonated.pdb". The entry is case sensitive.')
        ref_pdb = input("Enter the file name of the protonated reference PDB file: ")
        if not ref_pdb.endswith('.pdb'):
            ref_pdb += '.pdb'
        if check_exit(ref_pdb):
    ...
    try:
        ...
        print("All PLIF files moved to the PLIF_files folder in the current working directory.")
    except:
        print('Identical files were found in the output folder destination. No files were moved.')
    # End the timer
    end_time = time.time()
