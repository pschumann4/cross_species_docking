####################################################################################################
# This script allows you to get the protein binding pocket residues for a given ligand in a        #
# PDB file. The methods are based on the PLIP program                                              #
# (https://plip-tool.biotec.tu-dresden.de/plip-web/plip/index). The script will calculate the      #
# distance between the ligand and each residue in the PDB file and return the residues within a    #
# specified distance of the ligand. The default distance is 7.5 Angstroms.                         #
# The PDB file should NOT be protonated!                                                           #
...
def min_dist(pdb_file, ligand):
    ...
    res_coords = {}
    # Iterate through the lines of the PDB file on a per residue basis to calculate the residue centroids
    for line in lines:
        if line.startswith('ATOM'):
            ...
            # If the resnr is not in the res_coords dictionary, add it and set the value to a list containing the coordinates
        ...
        for ligand_coord in ligand_coords:
            for coord in coords:
                ...
                if dist < min_dist[resnr]:
                    min_dist[resnr] = dist
    # Save only the residue numbers that have a minimum distance less than 7.5 Angstroms
...
def get_bindingsite(pdb_file, ligand):
    ...
    if not ligand_coords:
        ...
        return None, None
    # Calculate the centroid of the ligand using the ligand coordinates and the centroid function
    ...
    for line in lines:
        if line.startswith('ATOM'):
            ...
            # Extract the coordinates from the line
            coords = [float(line[30:38]), float(line[38:46]), float(line[46:54])]
            # If the resnr is not in the res_centroids dictionary, add it and set the value to a list containing the coordinates
            if not resnr in res_coords:
                res_coords[resnr] = [coords]
            # If the resnr is in the res_centroids dictionary, append the coordinates to the list
            else:
                res_coords[resnr].append(coords)
    # Create a dictionary to store the residue centroids
    res_centroids = {}
    # Iterate through the res_coords dictionary and calculate the centroid of each residue
    for resnr, coords in res_coords.items():
        res_centroids[resnr] = centroid(coords)
    # Define the binding site distance (BS_DIST) in Angstroms
    BS_DIST = 7.5
    # Calculate the distance between the ligand centroid and the ligand atoms and store the maximum distance in max_dist
    max_dist = 0
    ...
    bindingsite_resnr = []
    # Iterate through the res_centroids and add the resnr to the bindingsite_resnr list if the distance between the ligand centroid and the residue centroid is less than or equal to the cutoff
    ...
    # Calculate the minimum distance between the ligand and each residue in the binding site
    min_dist_res = min_dist(pdb_file, ligand)
    # Remove resnr that are not matching with the min_dist_res list and the current bindingsite_resnr list
...
def get_pdb_bindingsite(pdb_file, bindingsite_res):
    ...
    for line in lines:
        ...
        if line.startswith('ATOM'):
            ...
            resnr = int(resnr)
            # If this resnr is equivalent to any resnr in the binding_site_resnr list, add the line to the new PDB list
...
pdb_file = "C:\\Users\\pschuman\\OneDrive - Environmental Protection Agency (EPA)\\Profile\\Desktop\\Folder\\AR\\2ama_modified.pdb"
ligand = "DHT"

def get_gridbox_coordinates(pdb_file, ligand):
    """This function calculates the gridbox size and coordinates for a given ligand in a PDB file.
    It does this by calculating the maximum distance between the ligand centroid and the ligand atoms.
    The gridbox size is then calculated as 4 times the maximum distance in each dimension (x, y, z).
    The gridbox coordinates are then calculated as the ligand centroid coordinates.
    """
    # Create a list to hold the coordinates of the ligand atoms
    ligand_coords = []
    # Read PDB file
    ...
    # Write the ligand centroid coordinates to a file and include the PDB file name as a header
    with open(pdb_file.split('.')[0] + '_gridbox_coords.txt', 'w') as f:
        f.write('PDB file: ' + pdb_file.split('.')[0] + '\n\nsize_x: '+ str(size) + '\nsize_y: ' + str(size) + '\nsize_z: ' + str(size) + '\n\ncenter_x: ' + str(ligand_centroid[0]) + '\ncenter_y: ' + str(ligand_centroid[1]) + '\ncenter_z: ' + str(ligand_centroid[2]))
...
def get_poc():
    import os
    # Ask the user if they would like to analyze a single PDB file or a directory of PDB files
    pdb_type = input('Analyze a single PDB file or a directory of PDB files? (single/directory): ')
    ...
    if pdb_type == 'single':
        ...
        while not os.path.exists(pdb_file):
            print(f'{pdb_file} does not appear to exist. Please enter a valid PDB file. The entry is case sensitive.')
            pdb_file = input('Enter the PDB file: ')
            if not pdb_file.endswith('.pdb'):
                pdb_file += '.pdb'
        ...
        with open(pdb_file, 'r') as f:
            lines = f.readlines()
        # Check if the ligand is present in the PDB file, if not ask the user to enter the ligand name again
        while ligand not in [line[17:20].strip() for line in lines if line.startswith('HETATM')]:
            print(f'{os.path.basename(pdb_file)} does not appear to contain the ligand {ligand}. Please try entering it again.')
        ...
        if check_multi_lig(pdb_file, ligand):
            print(f'ERROR: It appears that there are multiple {ligand} molecules in {pdb_file}.\nPlease remove all but one of the {ligand} molecules from the PDB file before using this function.')
            return
        # Check if the PDB file is protonated
        with open(pdb_file, 'r') as f:
            lines = f.readlines()
        if any([line[12:16].strip().startswith('H') for line in lines if line.startswith('ATOM')]):
            print(f'It appears that {pdb_file} is protonated.\nThe best results will be obtained by using an unprotonated PDB file.')
            ...
            while True:
                ...
                if continue_pdb == 'y':
                    ...
                    if not pdb_file.endswith('.pdb'):
                        pdb_file += '.pdb'
                        while not os.path.exists(pdb_file):
                            print(f'{pdb_file} does not exist. Please enter a valid PDB file.')
                            pdb_file = input('Enter the PDB file: ')
                            if check_exit(pdb_file):
                                return
        ...
        # Ask the user if they would like to retain the 'HETATM' records in the new PDB file
        retain_hetatm = input('Retain the ligand records in the new PDB binding pocket file? (y/n): ')
        # Check if the user made a typo and if so, ask them to enter the input again
        while retain_hetatm not in ['y', 'n']:
            print('Invalid input. Please enter either "y" or "n".')
            retain_hetatm = input('Retain the ligand records in the new PDB file? (y/n): ')
            if check_exit(retain_hetatm):
                return
        # If the user would like to retain the ligand records, transfer the ligand records from the original PDB file to the new PDB file
        if retain_hetatm == 'y':
            # Read the original PDB file
            with open(pdb_file, 'r') as f:
                lines = f.readlines()
                # Get the ligand records
                hetatm_records = [line for line in lines if line.startswith('HETATM') and line[17:20].strip() == ligand]
                # Add the ligand records to the new PDB file
                with open(os.path.join(os.path.dirname(pdb_file), os.path.basename(pdb_file).split('.')[0] + '_bindingsite.pdb'), 'a') as f:
                    f.write('\n')
                    for line in hetatm_records:
                        f.write(line)
            print('The ligand records have been retained in the new binding site PDB file.')
        else:
            print('The ligand records have been removed from the new binding site PDB file.')
        # Read the new PDB file
        with open(os.path.join(os.path.dirname(pdb_file), os.path.basename(pdb_file).split('.')[0] + '_bindingsite.pdb'), 'r') as f:
            lines = f.readlines()
        ...
        with open(os.path.join(os.path.dirname(pdb_file), os.path.basename(pdb_file).split('.')[0] + '_bindingsite.pdb'), 'w') as f:
            for line in lines:
                f.write(line)
        ...
        while output_gridbox not in ['y', 'n']:
            ...
            if check_exit(output_gridbox):
                return
        # If the user would like to output the gridbox coordinates, run get_gridbox_coordinates()
        if output_gridbox == 'y':
            get_gridbox_coordinates(pdb_file, ligand)
            # If there is a "details" folder, move the "bindingsite" PDBs and the gridbox coordinates to the "details" folder
            if os.path.exists(os.path.join(os.path.dirname(pdb_file), 'details')):
                shutil.move(os.path.join(os.path.dirname(pdb_file), os.path.basename(pdb_file).split('.')[0] + '_bindingsite.pdb'), os.path.join(os.path.dirname(pdb_file), 'details'))
                shutil.move(os.path.join(os.path.dirname(pdb_file), os.path.basename(pdb_file).split('.')[0] + '_bindingsite.poc'), os.path.join(os.path.dirname(pdb_file), 'details'))
                shutil.move(os.path.join(os.path.dirname(pdb_file), os.path.basename(pdb_file).split('.')[0] + '_gridbox_coords.txt'), os.path.join(os.path.dirname(pdb_file), 'details'))
                print('The gridbox coordinates and bindingsite files have been outputted to the "details" folder.')
            else:
                print('The gridbox coordinates and bindingsite files have been outputted to the current directory.')
    # If the user would like to analyze a directory of PDB files
        ...
        pdb_files = [os.path.join(pdb_dir, i) for i in os.listdir(pdb_dir) if i.endswith('.pdb')]
        # Iterate through the PDB files and generate a new PDB file with the binding site residues
        for pdb_file in pdb_files:
            ...
            with open(pdb_file, 'r') as f:
                lines = f.readlines()
            # Inform the user which PDB file is being processed. Extract the PDB file name.
            ...
            if ligand not in [line[17:20].strip() for line in lines if line.startswith('HETATM')]:
                print(f'{os.path.basename(pdb_file)} does not contain the ligand {ligand}. Skipping to the next PDB file.')
                continue
            # Check if the PDB file is protonated
            if any([line[12:16].strip().startswith('H') for line in lines if line.startswith('ATOM')]):
                print(f'It appears that {os.path.basename(pdb_file)} is protonated.\nSkipping to the next PDB file.')
            ...
            shutil.move(os.path.join(os.path.dirname(pdb_file), os.path.basename(pdb_file).split('.')[0] + '_bindingsite.pdb'), os.path.join(pdb_dir, 'binding_sites'))
            shutil.move(os.path.join(os.path.dirname(pdb_file), os.path.basename(pdb_file).split('.')[0] + '_bindingsite.poc'), os.path.join(pdb_dir, 'binding_sites'))
        # Inform the user where the new files were stored
        print(f'\nThe binding site PDB and POC files have been stored in {os.path.join(pdb_dir, "binding_sites")}')
        ...
        if combine == 'y':
            ...
            ref_poc = input("\nEnter the name of the reference .poc file so that it doesn't get added to the combined file: ")
            ...
            with open('all_pocs.poc', 'w') as o:
                for line in all_poc:
                    o.write(line)
        print('All of the query .poc files have been combined and saved as "all_pocs.poc" in the "binding_sites" folder.')
