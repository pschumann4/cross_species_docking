...
def mutate_res():
    print("\nThis function will perform point mutations in a PDB file by replacing specified residues with ALA.\n")
    # Ask the user for the working directory
    wd = input('Enter the path to the directory containing the PDB files you would like to mutate: ')
    # check that the directory exists
    while not os.path.isdir(wd):
        wd = input('Directory not found. Please enter the path to the directory containing the PDB files you would like to mutate: ')
    ...
    for pdb in os.listdir():
        ...
        with open(pdb, 'r') as f:
            lines = f.readlines()
            # For each residue number in the list of residues, remove all atoms that are not either N, CA, C, O, or CB
            for res in residues:
                for i in range(len(lines)):
                    if lines[i][22:26].strip() == res:
                        ...
                        if lines[i][12:16].strip() not in ['N', 'CA', 'C', 'O', 'CB']:
                            lines[i] = ''
            # Write the new PDB file using the same name as the original but with '_mutated' appended
    ...
    print('\nDone! All mutated structures have been moved to a new folder called "mutated".')
